...
public class GameController {
	...
	@RequestMapping(value = "/attack/{gameId}/{characterId}/{amount}", method = RequestMethod.POST)
	public Game attackCharacter(@PathVariable String gameId, @PathVariable String characterId, @PathVariable String amount)
					...
					if (character.getId() != characterToReview.getId()) {
						if (rule.isInRange(character.getPosition(), characterToReview.getPosition())) {
							if ((characterToReview.getLevel() - character.getLevel()) >= 5) {
								characterToReview.damage(Float.valueOf(amount)*0.5f);
							} else {
								characterToReview.damage(Float.valueOf(amount)*1.5f);
							}
						}
					}
				}
